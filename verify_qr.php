        ...
        if ($transaction_data && $transaction_data['status'] === 'APPROVED' && $transaction_data['reference'] === $transaction_uuid_for_redirect) {
            ...
            if ($current_tx && $current_tx['status'] === 'initiated') {
                ...
                if ($stmt_update->affected_rows > 0) {
                    add_system_message($conn, $current_tx['id'], "El pago ha sido completado a través de Wompi. Los fondos están en custodia.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['mark_as_shipped']) && $user_role === 'Vendedor' && $current_status === 'funded') {
        ...
        if ($update_stmt->execute()) {
             ...
             $_SESSION['error_message'] = "Error al actualizar el estado de la transacción.";
        }
    }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (wompiButton) {
                wompiButton.addEventListener('click', function() {
                    ...
                    checkout.open(function (result) {
                        const transactionResult = result.transaction;
                        if (transactionResult && transactionResult.status === 'APPROVED') {
                            const currentUrl = new URL(window.location.href);
                            currentUrl.searchParams.set('wompi_id', transactionResult.id);
                            currentUrl.searchParams.set('tx_uuid', '<?php echo $transaction['transaction_uuid']; ?>');
            ...
            if (document.getElementById('qr-reader')) {
                    ...
                    qrReader.stop().then(ignore => {
                         fetch('verify_qr.php', {
                            ...
                            body: JSON.stringify({
                                ...
                                transaction_uuid: '<?php echo $transaction_uuid; ?>'
                            })
                        })
                ...
                qrReader.start({ facingMode: "environment" }, config, onScanSuccess);
            }
